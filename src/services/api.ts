...
const auth = {
  ...
  async createInvite({ max_uses = null, expires_in_hours = 72, role = 'student', label = null }) {
...
const chats = {
  ...
  async addMember(chatId, targetUserId) {
    return request(`/chats/${chatId}/members/${targetUserId}`, { method: 'POST' })
  },
  ...
  connect() {
    ...
    this.ws.onclose = () => {
      if (this.shouldReconnect) {
        console.log(`WS отключён, переподключение через ${this.reconnectDelay}мс...`)
